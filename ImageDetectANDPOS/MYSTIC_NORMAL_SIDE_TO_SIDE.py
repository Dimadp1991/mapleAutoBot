...
if __name__ == '__main__':
    ...
    interval_buff = datetime.now() + pd.DateOffset(minutes=BUFF_TIME)
    interval_one_min_buff = datetime.now() + pd.DateOffset(seconds=55)
    ...
    while True:
            if auto_click_a and datetime.now() >= interval_side:
                ...
                # 2-7 colder , 3-1 black , 1-5 panda ,
            ...
            if auto_click_a and \
                ...
                # if datetime.now() >= interval_one_min_buff:
                #     pydirectinput.keyDown("t")
                #     pydirectinput.keyDown("t")
                #     interval_one_min_buff = datetime.now() + pd.DateOffset(seconds=55)
