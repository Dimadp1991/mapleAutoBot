...
if __name__ == '__main__':
    ...
    while True:
        ...
        job_five_skill_one = current_time + pd.DateOffset(seconds=32)
        job_five_skill_two = current_time + pd.DateOffset(seconds=26)
        job_five_skill_three = current_time + pd.DateOffset(seconds=70)
        ...
        while datetime.now() < current_two:
            ...
            if not keyboard.is_pressed('up') and not keyboard.is_pressed('down') and not keyboard.is_pressed('alt') and not keyboard.is_pressed('left') and not keyboard.is_pressed('right'):
                if datetime.now() > job_five_skill_one:
                    pydirectinput.keyUp("a")
                    ...
                    job_five_skill_one = datetime.now() + pd.DateOffset(seconds=32)
